...
export async function POST(req: Request) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!nome) nome = carryNome; else carryNome = nome;
      if (!statusGeralRaw) statusGeralRaw = carryStatusGeral; else carryStatusGeral = statusGeralRaw;
      if (!consultor) consultor = carryConsultor; else carryConsultor = consultor;
      if (!loja) loja = carryLoja; else carryLoja = loja;
      if (!cargo) cargo = carryCargo; else carryCargo = cargo;
